...
class Clipboard extends Module {
  ...
  onPaste(e) {
    ...
    setTimeout(() => {
      ...
      if(this.quill.options && this.quill.options.removePasteFormat) {
        delta = this.removeAttributes(delta)
      }
  ...
  removeAttributes(delta) {
    let tmpOps = delta.ops;
    if (tmpOps) {
        for(let i = 0; i < tmpOps.length; i++) {
          let tmpOp = tmpOps[i];
          if(tmpOp && tmpOp.attributes) {
            delete tmpOp['attributes']
              tmpOps[i] = tmpOp
          }
        }
      delta.ops = tmpOps;
